...
先用 5.1 + 5.2 拿到证据（经代理速度、VPS 本地速度、mtr 的 AS 段），再开工单：
...
1. traceroute/mtr 全程走 <实际 AS，如 NTT AS2914，节点 129.250.x.x>，
   未见宣传的 <应有 AS，如 联通 AS9929 的 218.105/219.158> 节点；
2. 从中国大陆经代理下载仅 <如 0.12> Mbps，而 VPS 本地到国际测速为 <如 47> Mbps，
   证明瓶颈在跨境线路而非机器带宽，与宣传 <如 50>Mbps 严重不符。
要求：①更换为宣传所述真实线路，或 ②按未使用时长退款。
```

> 维权只打「线路 AS 造假」+「带宽缩水」两点最硬（有客观证据）。
